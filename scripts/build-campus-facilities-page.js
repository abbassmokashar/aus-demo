...
@media(max-width:760px){.facilities-hero{min-height:620px;align-items:flex-end}.facilities-hero-content{padding:120px 0 48px}.facilities-hero h1{font-size:clamp(40px,12vw,58px)}.facilities-hero-media::after{background:linear-gradient(0deg,rgba(17,24,39,.94),rgba(17,24,39,.32) 78%)}.facilities-intro,.facilities-sports{grid-template-columns:1fr}.facilities-grid{grid-template-columns:1fr}.facility-card{display:block}.facility-card img{position:static;width:100%;height:auto;aspect-ratio:4/3;object-fit:cover}.facility-card-body{position:relative;z-index:1;padding:22px 20px}.facilities-sports-copy{position:relative;z-index:1;padding:28px 22px}.facilities-sports-gallery{position:relative;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:none;gap:14px}.facilities-sports-gallery img{position:static;display:block;width:100%;height:auto;min-height:0;aspect-ratio:4/3;object-fit:cover}.facilities-actions{display:grid}.facilities-actions a{width:min(100%,360px)}}
@media(max-width:480px){.facilities-hero{min-height:560px}.facilities-sports-gallery{grid-template-columns:1fr}.facilities-sports-gallery img{min-height:0}.facility-card{border-radius:14px}}
</style>`;
